...
def main():
    ...
    logging.info(f"Train: {train_df.shape}, Validation: {val_df.shape}")
    ...
    for epoch in range(start_epoch, config['EPOCHS']):
        ...
        val_loss, val_acc = evaluate_model(model, val_loader, device, mode="validation")
    ...
    logging.info(f"모델 학습 완료. 최적 모델이 저장되었습니다: {best_model_path}")
    logging.info(f"실험 로그가 다음 폴더에 저장되었습니다: {experiment_dir}")
