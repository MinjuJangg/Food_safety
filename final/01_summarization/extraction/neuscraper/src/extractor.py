...
def save_predictions(pred_nodes, threshold):
...
def inference_prob(args, model, corpus_data):
    data_process_fn = wrapped_commoncrawl_process_fn(args)
    dataset = SamplesDataset(corpus_data, data_process_fn)
    dataloader = DataLoader(
            dataset, batch_size=256, num_workers=0, collate_fn=content_extraction_collate_fn, drop_last=False
    ...
    for val_step, batch in enumerate(dataloader):
        ...
        primary_task_probs = output[:, :, 0]

        node_ids_cpu = node_ids_batch.cpu().numpy()
        probs_cpu = primary_task_probs.cpu().numpy()
        
        for batch_idx in range(len(urls)):
            curr_url_list = urls[batch_idx]
            if not curr_url_list:
                continue
            curr_url = curr_url_list[0]

            if curr_url not in predicted_nodes:
                predicted_nodes[curr_url] = {}

            doc_node_ids = node_ids_cpu[batch_idx]
            doc_probs = probs_cpu[batch_idx]

            for node_id, prob in zip(doc_node_ids, doc_probs):
                if node_id != 0:
                    predicted_nodes[curr_url][int(node_id)] = float(prob)

    return predicted_nodes

def get_text_and_probs(text_nodes_df, pred_nodes, url):
    pred_df = pd.DataFrame(pred_nodes[url].items(), columns=['TextNodeId', 'Prob'])
    text_pred_nodes_df = pd.merge(text_nodes_df, pred_df, how='left', on=['TextNodeId'])
    return text_pred_nodes_df.dropna().sort_values(['TextNodeId'], ascending=[False])